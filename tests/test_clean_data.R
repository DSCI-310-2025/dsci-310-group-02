...
test_that("clean_animal_shelter_data correctly recategorizes outcome types", {
  ...
  expect_equal(cleaned_df$outcome_type[3], "Not Adopted")
})


test_that("clean_animal_shelter_data calculates age_at_intake correctly", {
  expected_age <- as.numeric(difftime(test_df$intake_date[-3], test_df$dob[-3], units = "days")) / 365
  expect_equal(cleaned_df$age_at_intake, expected_age, tolerance = 0.1)
})
